...
class ImageEditService:
    ...
    @staticmethod
    def _is_token_retryable_upload_error(error: Exception) -> bool:
        """Whether image-upload failure is worth retrying with another token.

        Some upstream 403 responses on asset upload appear token/account-specific
        even when CF cookies and browser fingerprint are valid. In that case,
        switching to another token is safer than failing the whole edit request.
        """
        ...
        return status == 403 and "assetsuploadreverse" in message
        ...
        for attempt in range(max_token_retries):
            ...
            try:
                ...
                if rate_limited(e):
                    ...
                    continue
                if self._is_token_retryable_upload_error(e):
                    await token_mgr.record_fail(
                        current_token,
                        403,
                        "assets_upload_forbidden",
                    )
                    logger.warning(
                        f"Token {current_token[:10]}... asset upload forbidden (403), "
                        f"trying next token (attempt {attempt + 1}/{max_token_retries})"
                    )
                    continue
                raise
